...
import { addToCartF, removeCart } from '../redux/actions/cartActions';
...
const CartPageFemale = (props) => {
	...
	useEffect(() => {
		if (product_id) {
			dispatch(addToCartF(product_id, qty));
		}
	...
	return (
									...
									<tr>
										<td>
											<Figure>
												...
												<Figure.Caption style={{ textAlign: 'center' }}>
													<Nav.Link href={`/productf/${item.product}`}>{item.name}</Nav.Link>
										...
										<td style={{ padding: '100px' }}>
											<select
												value={item.qty}
												onChange={(e) =>
													dispatch(addToCartF(item.product, Number(e.target.value)))
												}
		...
		// 								<select
		// 									value={item.qty}
		// 									onChange={(e) => dispatch(addToCartF(item.product, Number(e.target.value)))}
		// 								>
		// 									{[...Array(item.inStock).keys()].map((a) => (
		// 										<option key={a + 1} value={a + 1}>
		// 											{a + 1}
